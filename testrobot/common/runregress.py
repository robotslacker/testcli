...
class Regress(object):
    ...
    def run(self):
        ...
        # 从Jenkins环境变量，如果没有，则本地模拟这些变量
        mEnvWORKSPACE = os.getenv("WORKSPACE")
        if mEnvWORKSPACE is None:
            try:
                repoRootDirectory = git.Repo(
                    "..", search_parent_directories=True).git.rev_parse("--show-toplevel")
                repoRootDirectory = repoRootDirectory.replace("\\", "/")
                os.environ["WORKSPACE"] = repoRootDirectory
            except Exception:
                # 无法获得测试程序目录，可能不是GIT环境
                self.logger.error(
                    "Not a valid GIT environment, failed to get Git Repo.")
                repoRootDirectory = os.path.abspath(os.path.join(
                    os.path.dirname(__file__), "../..", "..", ".."))
                if not os.path.exists(repoRootDirectory):
                    self.logger.error(
                        "Not a valid GIT environment, failed to get root directory.")
                    return
        self.logger.info("当前运行工作空间：【" + os.environ["WORKSPACE"] + "】")
        mEnvJOB_BASE_NAME = os.getenv("JOB_BASE_NAME")
        if mEnvJOB_BASE_NAME is None:
            mEnvJOB_BASE_NAME = "local-regression-test"
            os.environ["JOB_BASE_NAME"] = mEnvJOB_BASE_NAME
        mEnvBUILD_DISPLAY_NAME = os.getenv("BUILD_DISPLAY_NAME")
        if mEnvBUILD_DISPLAY_NAME is None:
            mEnvBUILD_DISPLAY_NAME = datetime.datetime.now().strftime('%Y%m%d%H%M%S')
            os.environ["BUILD_DISPLAY_NAME"] = mEnvBUILD_DISPLAY_NAME
        mEnvTWORK = os.getenv("T_WORK")
        if mEnvTWORK is None:
            mEnvTWORK = os.path.join(os.environ["WORKSPACE"], "build")
            os.environ["T_WORK"] = mEnvTWORK
        mEnvTEST_ROOT = os.getenv("TEST_ROOT")
        if mEnvTEST_ROOT is None:
            mEnvTEST_ROOT = os.path.join(os.environ["WORKSPACE"], "linkoopdb")
            os.environ["TEST_ROOT"] = mEnvTEST_ROOT
        ...
        if "GIT_BRANCH" in os.environ:
            gitTest_Branch = os.environ["GIT_BRANCH"]
            if gitTest_Branch.startswith("origin/"):
                gitTest_Branch = gitTest_Branch[len("origin/"):]
        else:
            try:
                gitTest_Branch = git.Repo(
                    "..", search_parent_directories=True).active_branch.name
            except Exception:
                # 无法获得branch名称
                self.logger.error(
                    "Not a valid GIT environment, failed to get GIT_BRANCH.")
                gitTest_Branch = "UNKNOWN"
        self.logger.info("当前运行GIT分支： 【" + gitTest_Branch + "】")

        # 用户没有指定， 使用JENKINS的默认值
        if "JOB_NAME" in os.environ:
            self.jobName = os.environ["JOB_NAME"]
        else:
            # 用户手工提交的JOB
            self.jobName = "USER-LOCALRUN"
        if "BUILD_NUMBER" in os.environ:
            self.buildNumber = os.environ["BUILD_NUMBER"]
        else:
            # 用户手工提交的JOB
            self.buildNumber = self.jobId
        self.logger.info("任务名称：【" + str(self.jobName) + "】")
        self.logger.info("任务编号：【" + str(self.buildNumber) + "】")
        ...
        for robotFile in robotFileList:
            ...
            if len(testCaseList.tests) <= 0:
                ...
                suite.remove_empty_suites(True)
                suite.visit(testCaseList)
                robotRelFile = robotFile.replace("\\", "/")
                if robotRelFile.find(os.environ["WORKSPACE"]) != -1:
                    robotRelFile = robotRelFile.replace(
                        os.environ["WORKSPACE"], "")
                self.ignoredTaskList.append(
                    {
                        "robotfile": robotFile,
                        "robotrelfile": robotRelFile,
                        "validcase": 0,
            ...
            # 只要找到一个robotId，就认为整个Robot都是这个robotId
            runLevel = None
            ...
            if m_RobotRelFile.find(os.environ["WORKSPACE"]) != -1:
                m_RobotRelFile = m_RobotRelFile.replace(
                    os.environ["WORKSPACE"], "")
        ...
        # 清理工作目录
        self.logger.info("将清理工作目录 【" + mEnvTWORK + "】， 该目录下所有文件都将会被清空...")
        if os.path.exists(mEnvTWORK):
            files = os.listdir(mEnvTWORK)
            for file in files:
                filePath = os.path.join(mEnvTWORK, file)
                if os.path.isdir(filePath):
                    # conf文件是系统配置环境用到的文件，不能删除
                    if os.path.basename(filePath) == "conf":
                        continue
        ...
        try:
            ...
            htmlTestResult.setTitle(
                "测试报告-" + self.jobName + "-" + str(self.buildNumber))
            htmlTestResult.setDescription(
                "最大进程数  : " + str(self.maxProcess) + '<br>'
            )
            ...
            # 生成报告
            htmlTestRunner = HTMLTestRunner(title="测试报告-" + str(self.jobId))
